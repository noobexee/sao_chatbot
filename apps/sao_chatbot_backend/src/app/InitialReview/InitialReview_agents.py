...
def call_typhoon(system_prompt, user_text):
    try:
...
def agent_criteria1_judge(extracted_entity: str, candidates: list, document_text: str):
    ...
    **Output Format (JSON):**
    ...
    """
    return call_typhoon(system_prompt, document_text)

# --- AGENT 2: Criteria 2 (SAO Authority Check) ---
def agent_criteria2_sao_authority(text):
    system_prompt = """
    ...
    **Output Format (JSON):**
    ...
    """
    return call_typhoon(system_prompt, text)

# --- AGENT 3: criteria 4 (Detailed Sufficiency Check) ---
def agent_criteria4_sufficiency(text):
    system_prompt = """
    ...
    **กฎเหล็ก (CRITICAL RULE):**
    - สำหรับช่อง "ชื่อหน่วยรับตรวจ (entity)" คุณต้อง **"คัดลอก (Copy & Paste)"** คำที่ปรากฏในเอกสารมาใส่ตรงๆ เท่านั้น! 
    - ห้ามเดาคำ ห้ามแก้คำผิด ห้ามแก้ตัวสะกดเด็ดขาด (แม้ว่าคำนั้นจะพิมพ์ผิด เช่น "โรวพยาบาล" ก็ต้องตอบว่า "โรวพยาบาล")
...
def agent_criteria6_complainant(text):
    system_prompt = """
    คุณคือ 'นายทะเบียน' หน้าที่คือตรวจสอบรายชื่อบุคคลในเอกสาร
    - *ต้องเป็นชื่อเฉพาะบุคคลเท่านั้น* ต้องมีคำนำหน้า (นาย, นาง, นางสาว, ยศ) ตามด้วยชื่อและนามสกุลจริง
    - *ห้าม* นับนามแฝงหรือชื่อสมมติ เช่น "พลเมืองดี", "ผู้หวังดี", "ชาวบ้าน", "ข้าราชการชั้นผู้น้อย" เป็นชื่อคนเด็ดขาด
    - หากพบ 'เลขประจำตัวประชาชน' (13 หลัก) ให้ดึงมาด้วย หากไม่พบให้ใส่ "" (ค่าว่าง)

    ตอบกลับเป็น JSON เท่านั้น:
    {
        "people": [
                { 
                  "name": "คำนำหน้า ชื่อ-นามสกุลจริง", 
                  "role": "ผู้ร้องเรียน" หรือ "ผู้ถูกร้องเรียน" หรือ "พยาน" หรือ "ผู้เกี่ยวข้อง",
                  "id_card": "เลขประจำตัวประชาชน 13 หลัก หรือ ค่าว่าง"
                }
            ]
    }
    """
...
def agent_criteria8_other_authority(text, criteria2_result=None):
    context_instruction = ""
