...
PROMPT_TEXT = """
  ...
  กรณีพิเศษ (ข้อความที่ไม่มีคำสั่งนำหน้า):
  - ข้อความที่ **ไม่มี** คำสั่งนำหน้าดังต่อไปนี้  
    ...
    ให้นำข้อความนั้นไปจัดไว้ **ท้ายสุดของเอกสารเท่านั้น** โดยปฏิบัติดังนี้อย่างเคร่งครัด:

    ขั้นที่ 1 — เพิ่มบรรทัดคั่นและหัวข้อนำต่อไปนี้ก่อนข้อความทั้งหมดในส่วนนี้:

    ---
    **ข้อความเพิ่มเติมจากฉบับที่ {{AMEND_VERSION}}) พ.ศ. {{AMEND_YEAR}}**

    ขั้นที่ 2 — นำข้อความมาวางต่อจากหัวข้อนั้น **โดยห้ามเปลี่ยนเลขข้อเดิม**

    ขั้นที่ 3 — ทุกวรรค/บรรทัดในส่วนนี้ **ทุกบรรทัด** ต้องลงท้ายด้วย:
    "(เพิ่มเติมโดย{{AMEND_VERSION}} พ.ศ. {{AMEND_YEAR}})"

    ตัวอย่างรูปแบบที่ถูกต้อง:
    ---
    **ข้อความเพิ่มเติมจากฉบับที่ Y) พ.ศ. Z**

    ข้อ X เนื้อหาของข้อนี้... (เพิ่มเติมโดยฉบับที่ Y พ.ศ. Z)
    วรรคสองของข้อ X... (เพิ่มเติมโดยฉบับที่ Y พ.ศ. Z)
    ข้อ X+1 เนื้อหาของข้อนี้... (เพิ่มเติมโดยฉบับที่ Y พ.ศ. Z)
"""
...
class DocumentManager:
    ...
    def edit_document(
        ...
        *,
        ...
        snapshot_source = self.repo.get_snapshot_sources(doc_id)
