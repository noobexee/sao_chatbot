        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-3 py-2 min-w-[280px]">
            ...
            <div className="space-y-1 pb-10">
              {history.map((item) => (
                <div key={item.id} className={`group relative flex items-center justify-between rounded-full hover:bg-[#e8eaed] transition-colors overflow-visible ${activeMenuId === item.id ? 'z-20 bg-[#e8eaed]' : 'z-0'}`}>
                  ...
                  <div className="relative shrink-0 pr-2">
                    <button type="button" data-menu-btn-id={item.id} onClick={(e) => toggleMenu(e, item.id)} className={`p-1 rounded-full hover:bg-gray-300 text-gray-50 transition-all ${activeMenuId === item.id ? "opacity-100 bg-gray-300 block" : "opacity-0 group-hover:opacity-100"}`}>
                    ...
                    {activeMenuId === item.id && (
                      <div data-menu-panel-id={item.id} className="absolute right-0 top-8 z-[999] w-40 overflow-hidden rounded-lg border border-gray-200 bg-[#f8f9fa] shadow-xl" onClick={(e) => e.stopPropagation()}>
                        <button type="button" className="flex w-full items-center gap-3 px-4 py-3 text-sm text-gray-800 hover:bg-gray-200 transition-colors text-left" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>View Details</button>
                        <button type="button" className="flex w-full items-center gap-3 px-4 py-3 text-sm text-red-600 hover:bg-red-100 transition-colors text-left" onClick={(e) => handleDelete(e, item.id)}>Delete</button>
                      </div>
                    )}
                  </div>
                </div>
